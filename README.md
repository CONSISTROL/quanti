...
> ⚠️ **关于成交模式（`exec_next_open`）的实测结论**：真实使用场景是"收盘后看信号、次日早盘操作"，引擎支持 `exec_next_open: true` 模拟（T日收盘信号 → T+1日开盘价买卖，先卖后买、卖出当日禁买）。实测（2015-2026全区间）：**优先级+1 时延迟成交反而更好**——总收益 +910.6% → +1078.1%，最大回撤 33.3% → 29.8%，胜率 41% → 46%（收盘价买入隐含"收盘追高"偏差，次日开盘常回落反而买得更便宜；震荡市受益明显），但 2024-2026 趋势市少赚（+387% → +229%，延迟一天少吃一天涨幅）。**而优先级+100 时延迟成交是灾难**——+1421.7% → +113.9%，回撤 → 57.2%（几乎每天高开追买159941、收盘信号卖出，追高杀跌双倍磨损）。结论：优先级+1 是"实际可执行场景"的最优配置（+1078%、回撤29.8%），**优先级+100（强制满仓单标的）在真实成交下极不稳定，不建议使用**。
