...
>
> ⚠️ **关于自选池优先级（`watchlist_priority`）的实测结论**：给 159941 设置优先级+2 反而更差（总收益 +786% → +762%，最大回撤 27.4% → **37.1%**，交易 245 → 474 笔）。原因：159941 振幅小（日均 1.16%）、趋势信号翻转快，强制优先持有导致频繁换仓，且 2022 纳指熊市死扛深回撤。**不建议设置优先级，让它自然参与强弱轮动即可**。引擎保留该功能仅供实验。
>
