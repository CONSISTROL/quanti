...
> 💡 HTML 报告中的大表格（操作记录/选股排名）支持**列筛选 + 分页**：点击表头 ▾ 打开筛选面板——数值列（价格/金额/盈亏等）按范围筛选，枚举列（方向/代码/名称）多选，长文本列（原因）包含搜索；表格上方显示已筛选行数可一键清除。超过 30 行的表格自动**分页**（每页 10/30/50/100 行可切换，‹ › 翻页，页码输入框+跳转按钮直接跳页），筛选后分页自动重算。
...
> ⚠️ **关于成交模式（`exec_next_open` / `buy_next_open`）的实测结论**：真实使用场景是"收盘后看信号、次日早盘操作"，引擎支持两种模拟：`exec_next_open`（买卖都延迟一天）与 `buy_next_open`（仅买入延迟）。实测（2015-2026全区间）：
