...
> 💡 HTML 报告中的大表格（操作记录/选股排名）支持**列筛选 + 分页**：点击表头 ▾ 打开筛选面板——数值列（价格/金额/盈亏等）按范围筛选，枚举列（方向/代码/名称）多选，长文本列（原因）包含搜索；表格上方显示已筛选行数可一键清除。超过 30 行的表格自动**分页**（每页 10/30/50/100 行可切换，‹ › 翻页），筛选后分页自动重算。
